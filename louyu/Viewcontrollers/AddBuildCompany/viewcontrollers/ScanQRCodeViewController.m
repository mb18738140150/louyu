...
//    }];
    ...
    if ([urlStr containsString:@"统一社会信用代码"]) {
        NSArray * infoArray = [urlStr componentsSeparatedByString:@";"];
        NSString * companyName = @"";
        NSString * legalPerson = @"";
        NSString * creadit = @"";
        for (NSString * infoStr in infoArray) {
            if ([infoStr containsString:@"统一社会信用代码"]) {
                creadit = [[infoStr componentsSeparatedByString:@":"] lastObject];
            }
            if ([infoStr containsString:@"名称"]) {
                companyName = [[infoStr componentsSeparatedByString:@":"] lastObject];
            }
            if ([infoStr containsString:@"法定代表人"]) {
                legalPerson = [[infoStr componentsSeparatedByString:@":"] lastObject];
            }
        }
        NSDictionary * infoDic = @{@"companyCredit":creadit,@"legalPerson":legalPerson,@"companyName":companyName};
        
        if (self.ScanComplateBlock) {
            self.ScanComplateBlock(infoDic);
        }
        [self.navigationController popViewControllerAnimated:YES];
        return;
    ...
    [session GET:urlStr parameters:@{} progress:^(NSProgress * _Nonnull downloadProgress) {
        ;
