...
- (void)initalTables
{
    ...
    "CREATE TABLE IF NOT EXISTS MoerduoList (id integer PRIMARY KEY autoincrement,partName text(128) NOT NULL,partId text(128) NOT NULL,userId text(32) NOT NULL);";
...
{
    return [self.readOperation getDownloadAudioInfos];
}

- (BOOL)deleteAllAudios
{
    return [self.writeOperation deleteAllAudios];
}
...
{
    return [self.readOperation getDownloadAudioList];
}
...
{
    return [self.writeOperation deleteAllAudioList];
}
