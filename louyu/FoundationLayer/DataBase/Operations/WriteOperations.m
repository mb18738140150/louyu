...
- (BOOL)deleteAllAudios
{
    ...
    isSuccess = [self.dataBase executeUpdate:@"DELETE FROM Moerduo"];
    return isSuccess;
}
...
{
    BOOL isSuccess = [self.dataBase executeUpdate:@"insert into MoerduoList (partName,partId,userId) values (?,?,?)",[dic objectForKey:kpartName],[dic objectForKey:kpartId],[NSString stringWithFormat:@"%@", [dic objectForKey:kUserId]]];
...
- (BOOL)deleteAllAudioList
{
    ...
    isSuccess = [self.dataBase executeUpdate:@"DELETE FROM MoerduoList"];
    return isSuccess;
}
